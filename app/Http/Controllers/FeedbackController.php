...
{
    /**
     * Display a listing of feedbacks with pagination and optional filtering.
     *
    ...
    {
        Log::info('Fetching feedbacks', ['request' => $request->all()]);
        ...
        try {
            $feedbacks = Feedback::with(['recipient', 'foodbank'])
                ->when($validated['recipient_id'] ?? null, fn($query, $id) => $query->where('recipient_id', $id))
                ->when($validated['foodbank_id'] ?? null, fn($query, $id) => $query->where('foodbank_id', $id))
                ->when($validated['rating'] ?? null, fn($query, $rating) => $query->where('rating', $rating))
                ->paginate(10);

            Log::info('Feedbacks fetched successfully', ['feedbacks' => $feedbacks]);

            return response()->json($feedbacks, 200);
        } catch (\Exception $e) {
            Log::error('Failed to fetch feedbacks', ['error' => $e->getMessage()]);
            return response()->json(['error' => 'Failed to fetch feedbacks', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            Log::info('Feedback deleted successfully', ['feedback_id' => $id]);
